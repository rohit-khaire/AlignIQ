...
def generate_remediated_export(remediated_path: str, format_type: str, company_name: str) -> tuple[bytes, str, str]:
    ...
    if format_type == "docx":
        ...
        pdf.multi_cell(190, 15, f"{clean_company} - Remediated Policies", align='C', fill=True)
        pdf.ln(10)
        ...
        for cat in data.get("remediated_categories", []):
            ...
            pdf.multi_cell(190, 10, cat_name)
            pdf.ln(2)
            ...
            for line in text.split('\n'):
                if line.strip() == '---':
                    ...
                    pdf.set_font("Helvetica", 'B', 12)
                    pdf.multi_cell(190, 8, line[3:].encode('latin-1', 'replace').decode('latin-1'))
                    pdf.set_font("Helvetica", '', 11)
                elif line.startswith('### '):
                    pdf.set_font("Helvetica", 'B', 11)
                    pdf.multi_cell(190, 7, line[4:].encode('latin-1', 'replace').decode('latin-1'))
                    pdf.set_font("Helvetica", '', 11)
                elif line.strip():
