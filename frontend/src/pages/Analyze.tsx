...
export const Analyze: React.FC = () => {
  ...
  // Clean up temporary files on server when page refreshes or mounts
  useEffect(() => {
    fetch(`${API_URL}/api/v1/compliance/reset`, { method: 'POST' }).catch(() => {});
  }, []);
          ...
          <AnimatePresence mode="wait">
            ...
            {appState === 'results' && (
              <motion.div 
                ...
                className="w-full"
              >
                <ReportDashboard report={reportData} onReset={handleClearFile} onAutoFix={() => setAppState('autofix')} />
              </motion.div>
            )}
            ...
            {appState === 'autofix' && (
              <motion.div 
                ...
                className="w-full"
              >
                <AutoFixer onBack={() => setAppState('results')} />
              </motion.div>
            )}
